...
void crossover ( int &seed )// chon lua cha me de lai ghep
{
  const double a = 0.0;
  const double b = 1.0;
  int mem;
...
void elitist ( )// luu tru lai nhung thanh vien tot nhat sau moi the he
{
  int i;
  double best;
  int best_mem;
...
void evaluate ( )// thuc hien danh gia sau tung the he 
{
  int member;
  int i;
  double x[NVARS+1];
...
int i4_uniform_ab ( int a, int b, int &seed )// random cac the he sinh ra trong quan the
{
  int c;
...
void initialize ( string filename, int &seed )// khoi tao gen nguon cua cac the he
{
  int i;
  ifstream input;
  int j;
...
void keep_the_best ( )// kiem tha, tim kiem va bo qua nhung doi tuong tot sau moi chu ky
{
  int cur_best;
  int mem;
  int i;
...
void mutate ( int &seed )// tao dot bien cho tung the he
{
  const double a = 0.0;
  const double b = 1.0;
  int i;
...
double r8_uniform_ab ( double a, double b, int &seed )// giao phoi ngau nhien 
{
...
void report ( int generation )// bao cao ket qua sau moi chu ky 
{
  double avg;
  double best_val;
  int i;
...
void selector ( int &seed )// thuc hien hcon lua doi utong lai gheps
{
  const double a = 0.0;
  const double b = 1.0;
  int i;
...
void Xover ( int one, int two, int &seed )//thu hien cheo hoa 2 me con 
{
  int i;
  int point;
  double t;
